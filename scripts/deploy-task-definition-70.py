#!/usr/bin/env python3
"""
Deploy Task Definition 70 with Updated Health Check Path

This script updates the ECS service to use task definition revision 70,
which changes the health check path from /health/minimal to /health/simple.
"""
    ...
    while True:
        ...
        if primary_deployment:
            ...
            if running_count == desired_count and len(deployments) == 1:
                ...
                print(f"✓ All {desired_count} tasks are running with task definition 70")
                return True
...
def verify_health_check_path(ecs_client, task_definition_arn):
    ...
    if health_check:
        ...
        if '/api/health/simple' in command:
            ...
            print(f"⚠ Warning: Health check path may not be /health/simple")
            return False
    else:
...
def save_deployment_record():
    ...
    record = {
        ...
        "task_definition": "multimodal-lib-prod-app:70",
        "task_definition_arn": TASK_DEFINITION_ARN,
        "cluster": CLUSTER_NAME,
        "service": SERVICE_NAME,
        "change": "Updated health check path from /health/minimal to /api/health/simple",
        "deployment_method": "ECS service update with force new deployment"
    }
    
    filename = f"task-definition-70-deployment-{timestamp}.json"
...
def main():
    print(f"\n{'='*60}")
    print("TASK DEFINITION 70 DEPLOYMENT")
    print(f"{'='*60}")
    print(f"Cluster: {CLUSTER_NAME}")
    print(f"Service: {SERVICE_NAME}")
    print(f"New Task Definition: {TASK_DEFINITION_ARN}")
    print(f"Change: Health check path /health/minimal → /api/health/simple")
    print(f"{'='*60}\n")
    ...
    try:
        ...
        if success:
            ...
            print("✓ Service is now using task definition 70")
            print("✓ Health check path updated to /api/health/simple")
            print("\nNext steps:")
