# IAM Roles and Policies for ECS Application
...
# Custom policy for ECS task execution (Secrets Manager, KMS)
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Condition = {
          StringEquals = {
            "cloudwatch:namespace" = "MultimodalLibrarian/${title(var.environment)}"
          }
        }
      },
      {
...
# Neptune access policy
resource "aws_iam_role_policy" "neptune_access" {
  name = "${var.project_name}-neptune-access"
  role = aws_iam_role.ecs_task.id
