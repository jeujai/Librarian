...
# Deploy AI-Enhanced The Librarian
# This script builds and deploys the full AI integration version

set -e

echo "🚀 Starting AI-Enhanced Deployment..."

# Configuration
AWS_REGION="us-east-1"
AWS_ACCOUNT_ID="591222106065"
ECR_REPOSITORY="multimodal-librarian-learning"
IMAGE_TAG="ai-enhanced"
...
# Step 2: Build the AI-enhanced Docker image
echo -e "${YELLOW}🏗️  Building AI-enhanced Docker image...${NC}"
docker build -f Dockerfile.ai-enhanced -t $ECR_REPOSITORY:$IMAGE_TAG .
...
# Step 5: Create new task definition with AI-enhanced image
echo -e "${YELLOW}📝 Creating AI-enhanced task definition...${NC}"
...
.containerDefinitions[0].environment += [
  ...
  {"name": "DEPLOYMENT_TYPE", "value": "ai-enhanced"},
  {"name": "AI_VERSION", "value": "gemini-2.5-flash-multimodal"}
]')

# Register the new task definition
echo "$NEW_TASK_DEF" > ai-enhanced-task-def.json
NEW_TASK_DEF_ARN=$(aws ecs register-task-definition --region $AWS_REGION --cli-input-json file://ai-enhanced-task-def.json --query 'taskDefinition.taskDefinitionArn' --output text)

echo -e "${GREEN}✅ New task definition registered: $NEW_TASK_DEF_ARN${NC}"

# Step 6: Update the ECS service
echo -e "${YELLOW}🔄 Updating ECS service with AI-enhanced task definition...${NC}"
...
echo -e "${YELLOW}🔍 Verifying AI-enhanced deployment...${NC}"
...
# Test AI endpoint
echo -n "  AI test: "
AI_RESPONSE=$(curl -s "$LB_URL/test/ai")
if echo "$AI_RESPONSE" | jq -e '.status == "success"' > /dev/null 2>&1; then
    echo -e "${GREEN}✅ AI Systems Operational${NC}"
else
    echo -e "${YELLOW}⚠️  AI Systems Initializing${NC}"
fi

echo ""
echo -e "${GREEN}🎉 AI-Enhanced deployment completed successfully!${NC}"
echo ""
...
echo "  Current: ~$80-150/month (Gemini 2.5 Flash - cost optimized)"
echo "  Note: Gemini 2.5 Flash is significantly cheaper than GPT-4"
echo ""
echo -e "${GREEN}🚀 Your Gemini 2.5 Flash-powered The Librarian is now live!${NC}"

# Cleanup
rm -f ai-enhanced-task-def.json
